...
```diff
 @@  Regular Expressions: Match WhitespacePassed  @@
```
...
```diff
 @@  Regular Expressions: Specify Upper and Lower Number of MatchesPassed @@
```
...
```diff
 @@ return a RegExp object that matches any string  beginning with and ending in the same vowel.  @@
```
//  ^ => first item matches:
...
return re;
the \1 means "match whatever was captured in the first capture group".

In other words, let's say the first capture group (meaning the first set of brackets) found an a, than that means, "we need another a here".

the . means any character and the * means any amount of the previous character, together .* means any amount of any character or basically "I don't care about this part, as long as the rest is true".
